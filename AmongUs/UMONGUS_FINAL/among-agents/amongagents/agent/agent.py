...
def _weighted_pick(choices, weights=None):
    """Pick uniformly unless a full weight vector is provided."""
    if not choices:
        raise ValueError("LLM choices are empty.")
...
class LLMAgent(Agent):
    def __init__(self, player, tools, game_index, agent_config, list_of_impostors, preselected_model=None):
        ...
        # Per-model routing: LLM_ROUTES_JSON = {"model_id": {"url": ..., "key": ...}, ...}
        # Falls back to LLM_API_URL / LLM_API_KEY (CSV multi-URL round-robin) if no match.
        routes = {}
        ...
        if raw_routes:
            try:
                ...
                print(f"[agent.py] WARN: LLM_ROUTES_JSON parse failed ({e}); falling back to global URL/KEY.")
    ...
    def _parse_action_response(self, response):
        ...
        match = re.search(pattern, response or "")
        if not match:
            return {}, (response or "").strip()
        return {
            "Condensed Memory": match.group(1).strip(),
            "Thinking Process": match.group(3).strip(),
    ...
    async def choose_action(self, timestep):
        ...
        for attempt in range(4):
            sections, output_action = self._parse_action_response(final_response)
