...
from amongagents.agent.agent_new import BeliefPredictOnlyLLMAgent, BeliefStateLLMAgent
...
class AmongUs:
    def __init__(
        ...
        UI=None,
        game_index=0,
        belief_state_models=None,
        belief_predict_only_models=None,
        required_models=None,
        predict_only_opponents=False,
        ...
        belief_state: bool
            Whether to use the belief-state LLM agent implementation.
        agent_config: dict
        ...
        self.belief_state_models = set(belief_state_models) if belief_state_models else set()
        self.belief_predict_only_models = set(belief_predict_only_models) if belief_predict_only_models else set()
        self.required_models = list(dict.fromkeys(required_models or []))
        self.predict_only_opponents = bool(predict_only_opponents)
        self.post_speech_belief_sweep = bool(post_speech_belief_sweep)
    ...
    def initialize_agents(self):
        ...
        mixed_mode = bool(self.belief_state_models) or bool(self.belief_predict_only_models)

        all_impostor_names = [p.name for p in self.players if p.identity == "Impostor"]

        if mixed_mode:
            self.list_of_impostors = list(all_impostor_names)
            incremental_impostor_names = []
        elif self.belief_state:
            self.list_of_impostors = list(all_impostor_names)
            incremental_impostor_names = None
        else:
            self.list_of_impostors = []
            incremental_impostor_names = self.list_of_impostors
        ...
        llm_player_indices = [
            ...
            if self.agent_config.get(player.identity) == "LLM"
        ]
        if len(self.required_models) > len(llm_player_indices):
            raise ValueError(
                "required_models has more entries than LLM player slots: "
                f"{len(self.required_models)} > {len(llm_player_indices)}"
            )
        required_by_index = {}
        if self.required_models:
            forced_slots = random.sample(llm_player_indices, len(self.required_models))
            forced_models = list(self.required_models)
            random.shuffle(forced_models)
            required_by_index = dict(zip(forced_slots, forced_models))

        def pick_model_for(player, player_index):
            if player_index in required_by_index:
                return required_by_index[player_index]
            if player.identity == "Crewmate":
                pool = self.agent_config["CREWMATE_LLM_CHOICES"]
                weights = self.agent_config.get("CREWMATE_LLM_WEIGHTS")
            else:
                pool = self.agent_config["IMPOSTOR_LLM_CHOICES"]
                weights = self.agent_config.get("IMPOSTOR_LLM_WEIGHTS")
            if weights and len(weights) == len(pool):
                return random.choices(pool, weights=list(weights), k=1)[0]
            return random.choice(pool)

        def make_llm_agent(player, player_index):
            chosen_model = pick_model_for(player, player_index)
            if mixed_mode:
                use_belief = chosen_model in self.belief_state_models
                use_predict_only = (not use_belief) and (
                    self.predict_only_opponents
                    or chosen_model in self.belief_predict_only_models
                )
            else:
                use_belief = bool(self.belief_state)
                use_predict_only = (not use_belief) and self.predict_only_opponents

            if use_belief:
                return BeliefStateLLMAgent(
                    player,
                    tools,
                    self.game_index,
                    self.agent_config,
                    self.list_of_impostors,
                    player_roster=[p.name for p in self.players],
                    preselected_model=chosen_model,
                ), True, False
            if use_predict_only:
                return BeliefPredictOnlyLLMAgent(
                    player,
                    tools,
                    self.game_index,
                    self.agent_config,
            ...
            impostor_list = (
                list(incremental_impostor_names)
                if incremental_impostor_names is not None
                else self.list_of_impostors
            )
            return LLMAgent(
                player,
                tools,
        ...
        for i, player in enumerate(self.players):
            ...
            if self.include_human and i == random_idx:
                ...
                kind = self.agent_config[player.identity]
                if kind == "LLM":
                    agent, used_belief, used_predict_only = make_llm_agent(player, i)
                    self.agents.append(agent)
                else:
                    self.agents.append(RandomAgent(player))
                print(
                    f"{i} Initializing player {player.name} with identity {player.identity} "
                    f"and LLM choice {self.agents[-1].model} "
                    f"(belief={used_belief}, predict_only={used_predict_only})"
                )

            if (
                incremental_impostor_names is not None
                and player.identity == "Impostor"
                and not used_belief
                and not used_predict_only
            ):
                incremental_impostor_names.append(player.name)
            ...
            self.summary_json[f"Game {self.game_index}"]["Player " + str(i+1)] = {
                ...
                "belief_state": used_belief,
                "belief_predict_only": used_predict_only,
                "required_model": self.agents[-1].model in self.required_models,
    ...
    async def belief_sweep(
        self,
        trigger,
        source_agent=None,
        source_action=None,
        include_source=False,
        listener_role_filter=None,
        commit=True,
        speech_event_id=None,
    ):
        if not self.post_speech_belief_sweep:
            return

        self.check_actions()
        self.update_map()
        phase = "Meeting phase" if self.current_phase == "meeting" else "Task phase"
        source_player = source_agent.player.name if source_agent is not None else None
        source_speech = str(source_action) if source_action is not None else None
        allowed_listener_roles = set(listener_role_filter or [])

        for listener_agent in self.agents:
            if not include_source and source_agent is not None and listener_agent is source_agent:
                continue
            if not listener_agent.player.is_alive:
                continue
            if allowed_listener_roles and listener_agent.player.identity not in allowed_listener_roles:
                continue
            if not hasattr(listener_agent, "update_belief_state"):
                continue

            all_info = listener_agent.player.all_info_prompt()
            await listener_agent.update_belief_state(
                all_info,
                phase,
                self.timestep,
                trigger=trigger,
                source_speech_player=source_player,
                source_speech=source_speech,
                commit=commit,
                speech_event_id=speech_event_id,
            )

    def _is_speech_action(self, action):
        return action is not None and getattr(action, "name", "") == "SPEAK"

    def _speech_event_id(self, agent, discussion_round):
        return (
            f"game={self.game_index}|step={self.timestep}|"
            f"round={discussion_round}|speaker={agent.player.name}"
        )
    ...
    async def meeting_phase(self):
        ...
        self.update_map()
        await self.belief_sweep(trigger="meeting_start", include_source=True)
        ...
        for round in range(self.game_config["discussion_rounds"]):
            ...
            for agent in self.agents:
                if 'homosapiens' in agent.model:
                    ...
                    self.is_human_turn = False
                self.check_actions()
                will_speak = agent.player.is_alive and any(
                    self._is_speech_action(candidate)
                    for candidate in agent.player.get_available_actions()
                )
                speech_event_id = self._speech_event_id(agent, round) if will_speak else None
                if speech_event_id:
                    setattr(agent, "pending_speech_event_id", speech_event_id)
                    await self.belief_sweep(
                        trigger="pre_speech",
                        source_agent=agent,
                        include_source=False,
                        listener_role_filter=("Crewmate",),
                        commit=False,
                        speech_event_id=speech_event_id,
                    )
                try:
                    action = await self.agent_step(agent)
                finally:
                    if hasattr(agent, "pending_speech_event_id"):
                        delattr(agent, "pending_speech_event_id")
                if self._is_speech_action(action):
                    if speech_event_id is None:
                        speech_event_id = self._speech_event_id(agent, round)
                    await self.belief_sweep(
                        trigger="post_speech",
                        source_agent=agent,
