...
def setup_experiment(experiment_name, LOGS_PATH, DATE, COMMIT_HASH, DEFAULT_ARGS):
    ...
    If experiment_name is provided, use it in the directory name.
    Otherwise, fall back to date-based exp index.
    """
    ...
    if experiment_name:
        base_name = sanitize_name(experiment_name)

        next_index = 0
        candidate = base_name
        while os.path.exists(os.path.join(LOGS_PATH, candidate)):
