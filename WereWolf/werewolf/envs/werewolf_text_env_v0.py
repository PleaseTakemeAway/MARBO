...
from MARBO.wolf.werewolf.helper.log_utils import Log
...
class WerewolfTextEnvV0(gym.Env):
    def __init__(self, **kwargs):
        ...
        self.game_phase_set = ['init', 'skill_wolf', 'skill_seer', 'skill_guard', 'skill_witch', 'skill_hunter',
                               'state_reconstruction', 'speech', 'speech_pk', 'vote', 'vote_pk', 'end_game']
    ...
    def reset(self, **kwargs):
        ...
        self._next_phase_after_reconstruction = None
        self._next_act_idx_after_reconstruction = None
    ...
    def next_phase(self, action):
        ...
        if self.phase == 'skill_wolf':
            ...
            self.game_log.append(
                Log(viewer=[self.SEER_IDX, ], source=self.current_act_idx, target=action_content,
                    content={'checked_identity': checked_identity},
                    day=self.day, time=self.get_time(), event=self.phase))
            ...
            if self.hunter_in_night:
                ...
                self._start_reconstruction_phase('speech')
        ...
        elif self.phase == 'state_reconstruction':
            assert action_type == 'state_reconstruction'
            self.game_log.append(
                Log(viewer=[self.current_act_idx], source=self.current_act_idx, target=-1,
                    content={'identity_labels': action_content},
                    day=self.day, time=self.get_time(), event=self.phase))
            if len(self.reconstruction_queue) > 0:
                self.current_act_idx = self.reconstruction_queue.pop(0)
            else:
                self.phase = self._next_phase_after_reconstruction
                if self._next_act_idx_after_reconstruction is not None:
                    self.current_act_idx = self._next_act_idx_after_reconstruction
                else:
            ...
            if len(self.speech_queue) > 0:
                ...
                next_vote_phase = 'vote' if self.phase == 'speech' else 'vote_pk'
                if next_vote_phase == 'vote_pk':
                    assert len(self.vote_queue) > 0
                else:
                    assert len(self.vote_queue) == 0
                    self.vote_queue = [idx for idx, live in enumerate(self.alive) if live == 1.]
                    assert len(self.vote_queue) > 0
                next_act_idx = self.vote_queue.pop(0)
                self._next_vote_phase = next_vote_phase
                self._start_reconstruction_phase(next_vote_phase, next_act_idx)
        ...
        if expelled_target == self.HUNTER_IDX and self.HUNTER_IDX != -1:
            ...
            for idx in self.WOLF_IDX:
                if self.alive[idx] == 1:
                    wolf_act_idx = idx
                    break
            self._start_reconstruction_phase('skill_wolf', wolf_act_idx)
    ...
    def get_observation(self):
        ...
        if self.phase == 'skill_wolf':
            ...
            valid_action = ("state_reconstruction", -1)
            ...
            raise ValueError
        if 'speech' not in self.phase and self.phase != 'state_reconstruction':
    ...
    def _get_reconstruction_phase_name(self, day_or_night):
        if day_or_night != 'day':
            return 'state_reconstruction'

        if self._next_phase_after_reconstruction in ('speech', 'speech_pk'):
            return 'pre_state_reconstruction'
        if self._next_phase_after_reconstruction in ('vote', 'vote_pk'):
            return 'post_state_reconstruction'
        return 'state_reconstruction'

    def get_phase(self, day, day_or_night, phase):
        if phase == 'state_reconstruction':
            phase = self._get_reconstruction_phase_name(day_or_night)
        return str(day) + '_' + day_or_night + '_' + phase

    def get_time(self):
        return f"Day {self.day} {'Daytime' if self.day_or_night == 'day' else 'Nighttime'}"
    ...
    def _start_reconstruction_phase(self, next_phase, next_act_idx=None):
        self._next_phase_after_reconstruction = next_phase
        self._next_act_idx_after_reconstruction = next_act_idx
        self.phase = 'state_reconstruction'
        self.reconstruction_queue = [idx for idx, live in enumerate(self.alive) if live == 1]
        self.current_act_idx = self.reconstruction_queue.pop(0)
    ...
    def trans_action_agt_to_env(self, action):
        ...
        action_type = action[0]
        action_content = action[1]
        if not action_type.startswith('speech') and action_type != 'state_reconstruction':
