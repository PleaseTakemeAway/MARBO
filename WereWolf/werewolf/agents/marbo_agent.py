...
import ast
import traceback
from MARBO.wolf.werewolf.agents.prompt_template_v0 import CON
from MARBO.wolf.werewolf.agents.output_schema import get_response_format
...
def _strip_code_fence(text):
    if not isinstance(text, str):
        return ""
    stripped = text.strip()
    match = re.search(r"```(?:json|javascript)?\s*(.*?)\s*```", stripped, flags=re.IGNORECASE | re.DOTALL)
    if match:
...
def _extract_role_labels_loose(text):
    if not isinstance(text, str):
        return {}
    labels = {}
    # Match even when roles are unquoted, e.g. "Player 1": Villager
    # Also handles partially broken keys like '"Player 9: Villager'
    for m in re.finditer(
        ...
        pid = m.group(1).strip()
        role = _clean_scalar_text(m.group(2))
        if pid:
            labels[f"Player {pid}"] = role if role else "Unknown"
    return labels


def _recover_speech_payload(raw_text):
    """
    Best-effort parser for malformed speech pseudo-JSON.
    Recovers:
      - Identity to Present
      - Identity Labels
      - Vote
    ...
    m = re.search(
        r'["\']?(Identity to Present|self_present|想要展示的身份)["\']?\s*:\s*["\']?([^,"\}\n]+)',
        text,
        flags=re.IGNORECASE,
    )
    if m:
        recovered["Identity to Present"] = _clean_scalar_text(m.group(2))

    m = re.search(
        r'["\']?(Vote|call_for_vote|归票)["\']?\s*:\s*["\']?([^,"\}\n]+)',
        text,
        flags=re.IGNORECASE,
    )
    if m:
        recovered["Vote"] = _clean_scalar_text(m.group(2))

    # Speech: allow missing colon after key (`"Speech" blah blah`)
    m = re.search(
        r'["\']?(Speech|speech|发言)["\']?\s*:?\s*(.*)$',
        text,
    ...
    if m:
        ...
        recovered["Speech"] = _clean_scalar_text(speech)

    # Prefer labels block after "Identity Labels", fallback to whole text
    labels_source = text
    labels_block = re.search(
        r'["\']?(Identity Labels|role_label|身份标签)["\']?\s*:\s*\{(.*?)(?=\}\s*,\s*["\']?(Vote|Speech|call_for_vote|归票|发言)|\}\s*$)',
        text,
        flags=re.IGNORECASE | re.DOTALL,
    )
    if labels_block:
        labels_source = labels_block.group(2)

    labels = _extract_role_labels_loose(labels_source)
    if labels:
        recovered["Identity Labels"] = labels
...
class MaktoAgent(LLMAgent):
    ...
    # Generation
    # =========================
    def _get_response_format(self, observation):
        return get_response_format(
            observation.get("phase"),
            identity=observation.get("identity"),
            player_count=len(observation.get("roles", [])) or 9,
            valid_action=observation.get("valid_action"),
        )

    def __vllm_generate(self, messages, response_format=None):
        ...
        request_kwargs = {
            ...
            "extra_body": self.generation_extra_body,
        }
    ...
    def _coerce_action_to_valid_action(self, identity, action, valid_action):
        if action in valid_action or identity != "Witch":
            return action
        ...
        heal = str(parsed.get("heal", "no")).strip().lower()
        poison = str(parsed.get("poison", "no")).strip().lower()
        if heal == "no" or poison == "no":
            return action

        candidates = [
            f"{{'heal':'{heal}', 'poison':'no'}}",
            f"{{'heal':'no', 'poison':'{poison}'}}",
            "{'heal':'no', 'poison':'no'}",
        ]
        for candidate in candidates:
            if candidate in valid_action:
                print(f"[Action Normalize] Witch cannot heal and poison in the same night. {action} -> {candidate}")
                return candidate
        return action
    ...
    def _generate_speech(self, observation, messages):
        global attempt_number
        raw_action = self.__vllm_generate(messages, self._get_response_format(observation))
    ...
    def _generate_vote(self, observation, messages, valid_action):
        phase = observation['phase']
        global attempt_number
        response_text = self.__vllm_generate(messages, self._get_response_format(observation))
        print(response_text)
    ...
    def _generate_action(self, observation, messages, valid_action):
        ...
        try:
            ...
            action = self._coerce_action_to_valid_action(observation["identity"], action, valid_action)
            assert action in valid_action, (
            ...
            print(f"[Action Parse Retry] raw_action before retry: {raw_action}")
            print(f"[Action Parse Retry] error: {type(e).__name__}: {e}")
    ...
    def parse_night_action(self, identity, raw_action):
        ...
        def normalize_player_value(value):
            text = "no" if value is None else str(value).strip()
            if text.lower() in ("none", "no", "否"):
                return "no"
        ...
        if identity == "Witch":
            heal = normalize_player_value(get_val(["heal", "解药"]))
            poison = normalize_player_value(get_val(["poison", "毒药"]))
            action_str = f"{{'heal':'{heal}', 'poison':'{poison}'}}"
            action_reason = get_val(["reason", "原因"], "")
        elif identity == "Werewolf":
            kill = normalize_player_value(get_val(["kill", "杀害"]))
            action_str = f"{{'kill':'{kill}'}}"
            action_reason = get_val(["reason", "原因"], "")
        elif identity == "Seer":
            check = normalize_player_value(get_val(["check", "查验"]))
            action_str = f"{{'check':'{check}'}}"
            action_reason = get_val(["reason", "原因"], "")
        elif identity == "Guard":
            guard = normalize_player_value(get_val(["guard", "守卫"]))
            action_str = f"{{'guard':'{guard}'}}"
            action_reason = get_val(["reason", "原因"], "")
        elif identity == "Hunter":
            shoot = normalize_player_value(get_val(["shoot", "击杀"]))
    ...
    def parse_speech(self, raw_action):
        ...
        if json_str is not None:
            try:
                ...
                content = {}
        if not content:
            content = _recover_speech_payload(raw_action)
